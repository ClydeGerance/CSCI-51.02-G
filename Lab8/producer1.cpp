...
int main(int argc, char *argv[]) {
    ...
    short chunk; // how much data was actually read (in bytes)
    int idx = 0;
    while (true) {
        wait(EMPTY);
        ...
        memcpy(shm_ptr, &chunk, sizeof(short)); // store chunk size (number of bytes read) at the beginning of shared memory
